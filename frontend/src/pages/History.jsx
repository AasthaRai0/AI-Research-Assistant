...
import { formatRelativeDate } from "../lib/mockData";

export default function History() {
  const { conversations, deleteConversation, documents } = useApp();
        ...
        <div className="mx-auto max-w-4xl px-6 py-8 lg:px-10">
              ...
              {filtered.map((c, i) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 flex items-center gap-1.5 text-xs text-ink-400">
                        <FileText size={11} />
                        <span className="truncate">{getDocName(c.docId)}</span>
                        <span>·</span>
                  ...
                  <button
                    ...
                    className="flex-shrink-0 rounded-lg p-2 text-ink-300 opacity-0 transition-all hover:bg-red-50 hover:text-red-500 group-hover:opacity-100"
          ...
          {filtered.length === 0 && (
            <div className="rounded-2xl border border-dashed border-ink-200 py-16 text-center text-sm text-ink-400">
              {conversations.length === 0 ? "No conversations yet." : `No chats match "${query}"`}
            </div>
          )}
        </div>
      </div>
